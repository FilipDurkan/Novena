...
(function (global, factory) {
  ...
  function scrollIntoView(element, { offset: offsetBy = 0 } = {}) {
    const parents = isVisible(element)
    ...
    UIkit.component = function (name, options) {
      ...
      if (!options) {
        if (isPlainObject(components$3[id])) {
          components$3[id] = components$3[`data-${id}`] = UIkit.extend(
            components$3[id]
          );
        }
      ...
      return (components$3[id] = components$3[`data-${id}`] =
        isPlainObject(options) ? opt : options);
    };
  ...
  async function transition(
    el,
    show,
    { content, duration, velocity, transition }
  ) {
    var _el$_wrapper;
    content =
      ((_el$_wrapper = el._wrapper) == null
        ? void 0
        : _el$_wrapper.firstElementChild) || $(content, el);
  ...
  function isSameSiteAnchor(a) {
    return ["origin", "pathname", "search"].every(
      (part) => a[part] === location[part]
    );
  }
  ...
  var drop = {
    ...
    connected() {
      ...
      this._style = (({ width, height }) => ({ width, height }))(
        this.$el.style
      );
    },
  ...
  function getViewport$1(el, target) {
    return offsetViewport(
      overflowParents(target).find((parent) => within(el, parent))
    );
  }
  ...
  var navbar = {
...

      {
        ...
        handler(e) {
          ...
          if (keyCode === keyMap.ESC) {
            ...
            (_active$targetEl = active.targetEl) == null
              ? void 0
              : _active$targetEl.focus();
          }
...

      {
        ...
        handler(e) {
          ...
          if (
            ...
            active.$el === e.target &&
            !this.toggles.some(
              (el) => active.targetEl !== el && matches(el, ":focus")
            )
          ) {
            e.preventDefault();
          }
    ...
    methods: {
      getActive() {
        return (
          includes(this.dropdowns, active == null ? void 0 : active.$el) &&
          active
        );
      },
  ...
  function isEqualList(listA, listB) {
    return (
      listA.length === listB.length && listA.every((el) => listB.includes(el))
    );
  }
  ...
  function hasSelectableText(el) {
    return (
      ...
      toNodes(el.childNodes).some(
        (el) => el.nodeType === 3 && el.textContent.trim()
      )
    );
  }
    ...
    methods: {
      show(index, force = false) {
        ...
        if (!force && stack.length > 1) {
          if (stack.length === 2) {
            var _this$_transitioner;
            (_this$_transitioner = this._transitioner) == null
              ? void 0
              : _this$_transitioner.forward(Math.min(this.duration, 200));
          }
  ...
  function backgroundCoverFn(prop, el, stops, props) {
    ...
    for (const prop of bgProps) {
      ...
      positions[prop] = `max(${getBackgroundPos(el, prop)},-${overflow}px) + ${
        positions[prop]
      }`;
    }
